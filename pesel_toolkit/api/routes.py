...
# PESEL legacy backend için MiniMax extractor — form_toolkit'ten reuse et
import sys as _sys
from pathlib import Path as _Path
_sys.path.insert(0, str(_Path(__file__).resolve().parents[2] / "form_toolkit"))
from core.minimax_extractor import extract_with_minimax, MinimaxError  # noqa: E402
...
def parse_document_endpoint():
    ...
    minimax_key = getattr(config, "MINIMAX_API_KEY", "") or os.environ.get("MINIMAX_API_KEY", "")
    minimax_group = getattr(config, "MINIMAX_GROUP_ID", "") or os.environ.get("MINIMAX_GROUP_ID", "")
    minimax_model = getattr(config, "MINIMAX_MODEL", "") or os.environ.get("MINIMAX_MODEL", "")
    missing_fields = []
    ...
    if minimax_key:
        try:
            ...
            mappings, missing_fields = extract_with_minimax(
                parse_result["raw_text"], field_map, minimax_key, **kwargs
            )
        except MinimaxError as exc:
            current_app.logger.error("MiniMax eşleştirme hatası: %s", exc)
            return jsonify({"error": f"MiniMax hatası: {str(exc)}"}), 500
        except Exception as exc:
            current_app.logger.error("MiniMax beklenmeyen hata: %s", exc)
