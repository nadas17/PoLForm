...
def client():
    ...
    app.config["APP_CONFIG"].MINIMAX_API_KEY = "test-minimax-key"
...
def test_parse_document_returns_missing_fields(client):
    """Mock extract_with_minimax → (mappings, missing_fields) tuple döner."""
    mock_mappings = [
        {
    ...
    with patch("api.routes.parse_document") as mock_parse, \
         patch("api.routes.extract_with_minimax") as mock_extract:
