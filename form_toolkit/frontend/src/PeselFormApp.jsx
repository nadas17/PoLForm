...
const MARITAL_STATUS_OPTIONS = [
  { id: "s4_status_single",   label: "Bekâr (kawaler / panna)" },
...
const NOTIFY_OPTIONS = [
  { id: "s6_notify_paper",      label: "Kâğıt (papierowa)" },
...
function TextInput({ id, label, placeholder, max, value, onChange }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-3 py-1.5
                    ...
                    focus-within:border-blue-500/50 transition-colors group">
      <label htmlFor={id}
        className="sm:w-48 flex-shrink-0 text-[11px] sm:text-xs font-medium uppercase tracking-wide
                   text-zinc-400 group-focus-within:text-blue-400 transition-colors
      ...
      <div className="flex-1 flex items-center min-w-0">
        <input id={id} type="text" maxLength={max} placeholder={placeholder}
          ...
          className="flex-1 min-w-0 bg-transparent text-sm font-mono text-zinc-50
  ...
  return (
    <label htmlFor={id} className="flex items-start gap-2.5 py-1.5 cursor-pointer group">
      <input type="checkbox" id={id} checked={!!checked}
        ...
        className="w-4 h-4 mt-0.5 rounded-sm border border-zinc-600 accent-blue-500
      ...
      {options.map((opt) => (
        <label key={opt.id} className="flex items-start gap-2.5 py-1.5 cursor-pointer group">
          <input type="radio" name={name}
            ...
            className="w-4 h-4 mt-0.5 accent-blue-500 cursor-pointer focus-ring" />
          <span className="text-sm text-zinc-400 group-hover:text-zinc-200
...
const CHECKBOX_FIELDS = new Set([
  "s2_sex_female", "s2_sex_male",
  "s2_citizenship_polish", "s2_citizenship_stateless", "s2_citizenship_other",
  "s4_status_single", "s4_status_married", "s4_status_divorced", "s4_status_widowed",
  "s5_event_marriage", "s5_event_divorce", "s5_event_annulment",
  "s5_event_spouse_death", "s5_event_spouse_death_found",
  "s6_notify_paper", "s6_notify_electronic",
]);

function normalizeCheckboxes(obj) {
  const result = { ...obj };
...
/* ─────────────────── ANA BİLEŞEN ─────────────────── */
export default function PeselFormApp({ onBack }) {
  const [tab, setTab] = useState("applicant");
  const [mobilePane, setMobilePane] = useState("form");
  const [data, setData] = useState(() => {
    try {
      const s = sessionStorage.getItem("pesel_data");
      return s ? normalizeCheckboxes(JSON.parse(s)) : {};
    } catch { return {}; }
  });
  const [missingFields, setMissingFields] = useState(() => {
    try {
      const s = sessionStorage.getItem("pesel_missing");
  ...
  /* Otomatik doldurma — varsayılanlar */
  useEffect(() => {
    setData(prev => {
      const next = { ...prev };
      let changed = false;
      ...
      if (!next.s7_legal_basis) {
        next.s7_legal_basis = "KARTA POBYTU - ZEZWOLENIE NA POBYT";
        changed = true;
      }
      if (!next.s8_date_day) {
        next.s8_date_day = String(today.getDate()).padStart(2, "0"); changed = true;
      }
      if (!next.s8_date_month) {
        next.s8_date_month = String(today.getMonth() + 1).padStart(2, "0"); changed = true;
      }
      if (!next.s8_date_year) {
        next.s8_date_year = String(today.getFullYear()); changed = true;
      }
      if (!next.s6_notify_paper && !next.s6_notify_electronic) {
        next.s6_notify_paper = true; changed = true;
      }
  ...
  useEffect(() => {
    setData(prev => {
      ...
      const hasSpouseData = next.s4_spouse_name || next.s4_spouse_maiden_name || next.s4_spouse_pesel;
      const noMaritalStatus = !next.s4_status_single && !next.s4_status_married
                           && !next.s4_status_divorced && !next.s4_status_widowed;
      if (hasSpouseData && noMaritalStatus) { next.s4_status_married = true; changed = true; }

      const noEventSelected = !next.s5_event_marriage && !next.s5_event_divorce
                            && !next.s5_event_annulment && !next.s5_event_spouse_death
                            && !next.s5_event_spouse_death_found;
      if (next.s4_status_married && noEventSelected) { next.s5_event_marriage = true; changed = true; }

      if (next.s6_epuap_address && !next.s6_notify_electronic) {
        next.s6_notify_paper = false;
        next.s6_notify_electronic = true;
        changed = true;
      }

      if (!next.s8_city && next.s1_city) { next.s8_city = next.s1_city; changed = true; }

      if (next.s2_citizenship_other_text && !next.s2_citizenship_polish
          && !next.s2_citizenship_stateless && !next.s2_citizenship_other) {
        next.s2_citizenship_other = true; changed = true;
      }

      if (next.s1_name && !next.s2_first_name) { next.s2_first_name = next.s1_name; changed = true; }
      if (next.s1_surname && !next.s2_surname) { next.s2_surname = next.s1_surname; changed = true; }

      return changed ? next : prev;
    });
  }, [data.s4_spouse_name, data.s4_spouse_maiden_name, data.s4_spouse_pesel,
      data.s4_status_married, data.s6_epuap_address, data.s1_city,
      data.s1_name, data.s1_surname, data.s2_citizenship_other_text]);
  ...
  const buildExport = useCallback(() => {
    const result = normalizeCheckboxes(data);
  ...
  const handleGeneratePDF = useCallback(async () => {
    ...
    try {
      ...
      const surname = payload.s2_surname;
      const name = payload.s2_first_name;
      const downloadName = surname && name
        ? `PESEL_${surname}_${name}_${today}.pdf`
        : `PESEL_${today}.pdf`;
      await generatePdf(payload, "pesel", downloadName);
  ...
  const handleDocImport = useCallback((importedData, missing) => {
    setData((prev) => ({ ...prev, ...normalizeCheckboxes(importedData) }));
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-zinc-950">
      ...
      <header className="flex-shrink-0 h-12 glass border-b border-white/[0.06] flex items-center px-3 sm:px-4 gap-2 sm:gap-3 z-30">
        ...
        <div className="flex flex-col sm:flex-row sm:items-baseline sm:gap-2 min-w-0">
          <span className="text-sm font-semibold text-zinc-50 truncate">PESEL Başvuru Formu</span>
          <span className="hidden sm:inline text-xs text-zinc-500">— Wniosek o nadanie numeru PESEL</span>
        </div>
        <div className="flex-1" />
        <ProgressIndicator filled={filledCount} total={totalFields} accent="blue" />
      </header>

      <TabBar tabs={TABS} active={tab} onSelect={setTab} accent="blue" />
      ...
      <SplitPane
            ...
            <div className="px-4 sm:px-5 py-3" role="tabpanel" id={`panel-${tab}`} aria-labelledby={`tab-${tab}`}>
              ...
              {tab === "personal" && (<>
                <div className="glass rounded-xl p-4 mb-3">
                  ...
                  <div className="mt-3 pt-2 border-t border-white/[0.06]">
                    <span className="text-xs text-zinc-400 font-medium uppercase tracking-wide block mb-1">Cinsiyet (Płeć)</span>
                    <RadioGroup name="cinsiyet" options={SEX_OPTIONS} value={data} onChange={handleChange} />
                  </div>
                  <div className="mt-3 pt-2 border-t border-white/[0.06]">
                    <span className="text-xs text-zinc-400 font-medium uppercase tracking-wide block mb-1">Doğum Tarihi (Data urodzenia)</span>
                ...
                <div className="glass rounded-xl p-4 mb-3">
                  ...
                  <div className="mt-3 pt-2 border-t border-white/[0.06]">
                    <span className="text-xs text-zinc-400 font-medium uppercase tracking-wide block mb-1">Vatandaşlık (Obywatelstwo)</span>
                    <RadioGroup name="vatandaslik" options={CITIZENSHIP_OPTIONS} value={data} onChange={handleChange} />
                ...
                <div className="glass rounded-xl p-4 mb-3">
                  ...
                  <div className="mt-2">
                    <span className="text-xs text-zinc-400">Son Kullanma Tarihi</span>
                    <div className="grid grid-cols-3 gap-3 mt-1">
                      {PASSPORT_FIELDS.filter(f => f.group).map(f =>
                        <TextInput key={f.id} {...f} value={data[f.id]} onChange={handleChange} />
                      )}
              ...
              {tab === "marital" && (<>
                <div className="glass rounded-xl p-4 mb-3">
                  ...
                  <RadioGroup name="medeni" options={MARITAL_STATUS_OPTIONS} value={data} onChange={handleChange} />
                </div>
                ...
                <div className="glass rounded-xl p-4 mb-3">
                  ...
                  <RadioGroup name="evlilik_olayi" options={EVENT_OPTIONS} value={data} onChange={handleChange} />
                  <div className="mt-3 pt-2 border-t border-white/[0.06]">
                    <span className="text-xs text-zinc-400">Olay Tarihi</span>
                    <div className="grid grid-cols-3 gap-3 mt-1">
              ...
              {tab === "address" && (<>
                <div className="glass rounded-xl p-4 mb-3">
                  ...
                  <RadioGroup name="bildirim" options={NOTIFY_OPTIONS} value={data} onChange={handleChange} />
                ...
                <div className="glass rounded-xl p-4 mb-3">
                  ...
                  <div className="mt-2">
                    <span className="text-xs text-zinc-400">İmza Tarihi</span>
                    <div className="grid grid-cols-3 gap-3 mt-1">
                ...
                <div className="glass rounded-xl p-4 mb-3">
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-wrap gap-2">
                      <button onClick={handleExportJSON} type="button"
                        className="bg-blue-500 text-white px-4 py-2 rounded-lg text-sm font-medium
                                   hover:bg-blue-400 transition-colors focus-ring">
                        JSON İndir
                      </button>
                    ...
                    <div>
                      ...
                      <pre className="bg-zinc-950 border border-white/[0.06] rounded-lg p-3 text-xs font-mono text-blue-400
        ...
        preview={<PdfPreview data={data} familyData={null} formType="pesel" />}
      />
